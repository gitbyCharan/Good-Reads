...
$dis_ar = json_decode($discount);

$book_dis = "";

foreach ($dis_ar as $single_ar) {

    $book_dis .= "discount < " . $single_ar . ' or ';

}
$book_dis = trim($book_dis, 'or ');
...
<div class="wrapper">
    ...
    if (count($dis_ar) == 0) {
        $select_Cat = "SELECT * FROM `books` limit 12";
    } else {
        $select_Cat = "SELECT * FROM `books` WHERE $book_dis";
    }
   // echo $select_Cat;
